...
plugin before it. This file is the working rules for HeliosPortal. Anything local to one machine lives
in the gitignored `CLAUDE.local.md`, never here: this file is public.

## Project

Progressive web app with an address book for many Helios Advance boards. A user keeps one icon rather than one per board. It talks to a board only through the public JSON API and its OpenAPI description, so it never depends on engine internals. One developer, six repositories under the `HeliosBBS` organisation
